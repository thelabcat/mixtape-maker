...
Find the optimal combination of music trakcs to fit on your mixtape size.
...
parser = argparse.ArgumentParser(
    description="Find the optimal combination of tracks to fit on your mixtape. Designed for use with the CR-669 Portable Cassette Player and Recorder AKA Phillips TAR5109.",
    epilog="S.D.G."
)
...
parser.add_argument("-e", "--end-margin", type=int, default=5 * 60, help="Margin of silence (in seconds) at the end to prevent looping")
...
"""Command to run in order to get the duration of a media file, suffixed by the path"""

EXTENSIONS = "mp3", "wma"
"""Filename extensions supported by the player"""
...
def get_duration(f: str) -> float:
    """Determine the duration of a media file via FFmpeg"""
    return float(subprocess.run(DURATION_COMMAND + [f], capture_output=True, encoding="utf-8").stdout.strip())
...
def get_list_duration(filelist: iter[str]) -> float:
    """Calculate the duration of a track file list, with gaps"""
    return sum(durations_sec[f] for f in filelist) + args.gap * (len(filelist) - 1)
...
for side_name, side in sides.items():
    ...
    AudioSegment.silent((TAPE_SIDE_SECONDS - get_list_duration(side) + args.end_margin)
                        * 1000).export(op.join(folder, f"{i * 2 + 1:0{digits}} blank gap.mp3"))
